...
const UploadField: FunctionComponent<Props> = ({
  ...
  useEffect(() => {
    ...
    if (onFinish && fileList.every((file) => file.status === 'done' || file.status === 'error')) {
      onFinish();
    }
  ...
  const uploadToStorage = ({ file, onSuccess }: RcCustomRequestOptions) => {
    ...
    uploadTask.on(
      ...
      () => {
        message.error(t<string>('attachment.error'));
        updateFileList((prev) =>
          prev.map((savedFile) => {
            if (savedFile.uid === file.uid) {
              return {
  ...
  return (
    <>
      ...
      >
        <Button icon={<UploadOutlined />}>{t<string>('common:button.upload')}</Button>
      </Upload>
    </>
  );
};
